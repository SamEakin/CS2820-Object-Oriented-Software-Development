...
public abstract class Gate {
	 ...
	 *  @param new the new logic value of this gate's output
	 */
	public String printValue(int oldv, int newv) {
		// was FALSE
		if (oldv == 0){
			if (newv == 0){
				return "|    ";
	...
	public static void initPrint(float i) {
		printInterval = i;
		Simulation.schedule(0.0f, (float t) -> printGates() );
	 ...
	 * @see printInterval
	 */
	private static void printGates(float t, int oldv, int newv) {
		for( Gate g: TernaryLogic.gates ) {
			System.out.print( " " + g.printValue(oldv, newv) );
		}
